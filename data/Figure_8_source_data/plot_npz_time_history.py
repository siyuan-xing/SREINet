...
def main() -> None:
    ...
    fig, axes = plt.subplots(3, 3, figsize=(7.0, 4.4), sharex=False, facecolor="none")
    ...
    # Place the shared legend inside the first panel so the full canvas width
    # can be used by the 3 x 3 subplot grid.
    axes[0, 0].legend(
        loc="upper right",
        frameon=False,
        fontsize=FS_SMALL,
    ...
    fig.tight_layout(pad=0.35, w_pad=0.7, h_pad=0.6)
    fig.savefig("triple_pendulum_npz_time_history.png", dpi=600, transparent=True)
