...
solution = solve_ivp(DNLS_ode, t_span, initial_state, t_eval=t_eval)
...
cax1 = ax1.imshow(heat_map_dx, aspect='auto', extent=[0, 80, -25, 25], cmap='cmo.delta')
...
cax2 = ax2.imshow(heat_map_pred_dx, aspect='auto', extent=[0, 100, -25, 25], cmap='cmo.delta')
...
cax3 = ax3.imshow(np.abs(heat_map_dx-heat_map_pred_dx), extent=[0, 100, -25, 25], aspect='auto', cmap='cmo.delta')
...
pos = [pos.x0-0.02, pos.y0, pos.width, pos.height]  # Shift the colorbar to the left by 0.05
...
#cbar_1.set_ticks([0, 1e-4, 2e-4, 3e-4, 4e-4, 5e-4])
...
ax3.set_xticks(ticks=[0, 20, 40, 60, 100])
...
plt.tight_layout()
#plt.show()
plt.savefig("DNLS_50_node_results.png", dpi=600)
