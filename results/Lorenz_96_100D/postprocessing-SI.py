...
solution = solve_ivp(lorenz96_ode, t_span, initial_state, t_eval=t_eval, method='DOP853')

MY_MODEL = 'Lorenz_96' 
LORENZ96_DIM = 100  # You can change this value to your desired number of elements
data_initial_conditions = [initial_state]

myDG = DG.DataGenerator(data_initial_conditions,T=T, dt=dt, derivative_mode="exact")
t_arr, x_original, dx_original, guess_highest_order_polynomial = myDG.generate_dataset_by_model_name(MY_MODEL, method='DOP853')

import matplotlib.pyplot as plt

#plt.figure(figsize=(10, 6))
#plt.plot(t_arr, x_train[:,0], 'k', label='True')
#plt.plot(t_arr, solution.y[0,:], 'r--', label='pred_dxicted')
...
#cax9 = ax9.imshow(np.abs(heat_map_x-heat_map_pred_x), extent=[0, 20, 0, 100], aspect='auto', cmap='cmo.tempo')
...
cax10 = plt.imshow(np.abs(heat_map_x-heat_map_pred_x), extent=[0, 20, 0, 100], aspect='auto', cmap='cmo.tempo')
...
#plt.text(-0.05, -0.1, '(b)', size=14, weight='bold', fontfamily='Arial')
...
ax12.set_yticks([-5,0, 5, 10])
ax12.set_xticks([0, 5,10, 15])
...
ax13.set_yticks([-5,0, 5, 10])
ax13.set_xticks([0, 5,10, 15])
