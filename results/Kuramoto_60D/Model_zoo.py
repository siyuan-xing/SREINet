...
def DSG(t, x, C=1.0, N=10):
    ...
    un_ddot = C * (un_plus_one + un_minus_one - 2 * un) - np.sin(un)

    #impose boundary conditions
    #u_{-1} =u_{0}
    un_ddot[0] = C * (un[1] + un[0] - 2 * un[0]) - np.sin(un[0])

    #u_{N+1} =u_{N}

    un_dot[-1]  = C * (un[-1] + un[-2] - 2 * un[-1]) - np.sin(un[-1])

    return np.concatenate([un_dot, un_ddot])


def discrete_phi_quartic(t,x, C=2.0, N=100):
    #discrete phi^4 equation
    # N: the number of grid points
    un = x[:N]
    un_dot = x[N:]
    ...
    un_ddot = C * (un_plus_one + un_minus_one - 2 * un) + 2*(un - un**3)

    #impose boundary conditions
    #u_{-1} =u_{0}
    #un_ddot[0] = C * (un[1] + un[0] - 2 * un[0]) + 2 * (un[0] - un[0]**3)

    #u_{N+1} =u_{N}
    #un_ddot[-1] = C * (un[-1] + un[-2] - 2 * un[-1]) + 2 * (un[-1] - un[-1]**3)
    return np.concatenate([un_dot, un_ddot])
...
def DNLS(t,un, C=2.0, N=100):
    ...
    #impose boundary conditions
    #u_{-1} =u_{0}
    #un_dot[0] = 1j*C * (un[1] + un[0] - 2 * un[0]) + 1j*(un[0]*np.abs(un[0])**2)

    #u_{N+1} =u_{N}
    #un_dot[-1] = 1j*C * (un[-1] + un[-2] - 2 * un[-1]) + 1j*(un[-1]*np.abs(un[-1])**2)
...
def DNLS_split(t,un, C=2.0, N=100):
    ...
    #impose boundary conditions
    #u_{-1} = 0
    #an_dot[0] = -C*(bn[1]   - 2*bn[0]) - bn[0]*(an[0]**2 +bn[0]**2)
    #bn_dot[0] = C*(an[1]  - 2*an[0]) + an[0]*(an[0]**2 +bn[0]**2)

    #u_{N+1} = 0
    #an_dot[-1] = -C*( bn[-2] - 2*bn[-1]) - bn[-1]*(an[-1]**2 +bn[-1]**2)
    #bn_dot[-1] = C*(  an[-2] - 2*an[-1]) + an[-1]*(an[-1]**2 +bn[-1]**2)
