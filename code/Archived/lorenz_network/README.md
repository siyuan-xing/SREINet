...
but focuses on pairwise-coupled dynamical systems over homogeneous Erdős–Rényi
networks.  No higher-order interactions are included.

## Contents

- `network_generator.py`: helper utilities to sample connected Erdős–Rényi graphs
  and visualise their structure.
- `coupling.py`: reusable diffusive coupling term for scalar node variables.
- `lorenz_network.py`: Lorenz oscillator dynamics with diffusive coupling on the
  x-coordinate.
- `lotka_volterra_network.py`: Predator–prey dynamics with diffusive coupling for
