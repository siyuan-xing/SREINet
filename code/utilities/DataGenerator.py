...
import numpy as np
"""
This module is used to generate data for the ODE models.
...
class DataGenerator:
    ...
    def __init__(self, initial_conditions:np.array, T:float=10.0, dt:float=0.1, t0:float=0.0, noise_level:float=0.0, derivative_mode="exact", random_state=None):
        """
    ...
    def generate_ode_traj_with_ic(self, model_ode, x0, derivative_mode="exact", *args, **kwargs):
        ...
        Args:
            ...
            random_state (int, optional): The random state used for random number generation. Defaults to None.
            args: Additional arguments to pass to the model_ode function.
    ...
    """
        ...
        # Apply noise using the apply_noise_np function
        x_train_noisy = apply_noise_np(x_train, self.noise_level, self.random_state)
        dx_train_noisy = apply_noise_np(dx_train, self.noise_level, self.random_state)
        
        return x_train_noisy, dx_train_noisy



@staticmethod
def apply_noise_np(data, noise_level, random_state):
    """Adds Gaussian white noise to a NumPy array based on a noise level.

    Args:
        data (np.ndarray): The data to which noise should be added.
        noise_level (float): The noise level as a function of the standard deviation of the data.
