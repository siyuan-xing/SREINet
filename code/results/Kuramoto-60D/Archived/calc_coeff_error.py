...
for line in lines:
    ...
    if match:
        ...
        pattern = r'([+-]?\d+\.\d+|\d+)(?=\*\s*sin\([x]\d+\)|\*\s*cos\([x]\d+\)|\s*\+\s*|\s*-\s*|$)'
        coeffs = re.findall(pattern, rhs)

        coeffs = [float(c) for c in coeffs]

        first_coefficients.append(coeffs[0])
        #sort the coefficients
        coeffs=sorted(coeffs[1:])
...
first_coefficients = np.array(first_coefficients)
coeff_matrix = np.array(coefficients)

coeff_error1 = np.abs(first_coefficients - np.linspace(-5,5,60))
coeff_error2 = np.abs(coeff_matrix - real_coeff)
coeff_error = np.concatenate((coeff_error1, coeff_error2.flatten()))
print("average coefficient error: ", np.mean(coeff_error))
# print("average coefficient error: ", (coeff_error1+coeff_error2)/2)
