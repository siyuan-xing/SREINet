...
real_coeff = np.array([1.0, 1.0, 1.0, 8.0])
...
for line in lines:
    ...
    if match:
        ...
        coeffs += [float(constant)]  # Add the constant term to the list of coefficients
        #sort the coefficients
        if len(coeffs) == 3:
            coeffs.append(1.0) # 1.0 was not detected in the file
