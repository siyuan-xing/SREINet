...
fig = plt.figure(figsize=(my_size*2, my_size/2))
...
ax2.set_position([ax2.get_position().x0, ax2.get_position().y0 + 0.03, ax2.get_position().width, ax2.get_position().height])
ax2.plot(t_arr, sim_amp[node_num//2-1,:],'k', linewidth=2.0)
...
ax4.set_position([ax4.get_position().x0, ax4.get_position().y0 + 0.03, ax4.get_position().width, ax4.get_position().height])
ax4.plot(t_arr, pred_amp[node_num//2-1, :],'k', linewidth=2.0)
ax4.set_xlim(0,data_T)
ax4.set_ylim(-0.2, 1.0)
#ax4.set_xlabel(r'Time, $t$')
#ax4.set_ylabel(r'Derivative, $\dot{x}_1$')
ax4.set_xticks([0, 60, 120, 180])
ax4.set_yticks([0, 0.5])
#add a vertical dash line at the middle of x axis
#ax4.axvline(x=reference_line, color='k', linestyle='--')

#ax2.text(-0.05, -0.8, '(a)', transform=ax2.transAxes, size=14,  fontweight='bold')
#ax4.text(-0.05, -0.8, '(b)', transform=ax4.transAxes, size=14,  fontweight='bold')
