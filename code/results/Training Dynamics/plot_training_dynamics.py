...
PLOT_TITLES = ['$\phi_4$', 'Kuramoto', 'Hindmarsh-rose']
...
def plot_files(paths, loss_mode, output_path, show_plot):
    ...
    fig, axes = plt.subplots(1, len(paths), figsize=(9,3), sharey=False)
    if len(paths) == 1:
        axes = [axes]
    ...
    for idx, (ax, path) in enumerate(zip(axes, paths)):
        ...
        ax.set_title(title if title is not None else path.stem.replace("_", " "))
