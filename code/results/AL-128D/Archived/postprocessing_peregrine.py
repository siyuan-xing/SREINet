...
gs = fig.add_gridspec(5, 5, width_ratios=[20, 3, 20, 3, 1], hspace=0.1, wspace=0.1)
...
ax6.set_ylabel(r'$|u_{1}(t)|$', rotation=0)
ax6.yaxis.set_label_coords(-0.15, 0.3)  # Move the label slightly to the left
#ax6.set_yticks([-5, 5])
#reference_line = 25 
...
ax1.set_position([ax1.get_position().x0-0.021, ax1.get_position().y0 + 0.02, ax1.get_position().width, ax1.get_position().height])
...
ax2.set_position([ax2.get_position().x0-0.021, ax2.get_position().y0 + 0.02, ax2.get_position().width, ax2.get_position().height])
