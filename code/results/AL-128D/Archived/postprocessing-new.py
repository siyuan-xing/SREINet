...
#training data
x_train = train_data[:N,0:dim]
dx_train = train_data[:N,dim:2*dim]

pred_dx = train_data[:N,2*dim:3*dim]
...
solution = solve_ivp(AL_ode, t_span, initial_state, t_eval=t_eval)
...
cax3 = ax3.imshow(np.abs(heat_map_dx-heat_map_pred_dx), extent=[0, 500, -32, 32], aspect='auto', cmap='cmo.delta')
...
plt.tight_layout()
#plt.show()
plt.savefig("AL_64_node_results.png", dpi=600)
